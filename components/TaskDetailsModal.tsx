...
interface TaskDetailsModalProps {
  ...
  allowManagement?: boolean; // New prop to control edit/upload features
}

const TaskDetailsModal: React.FC<TaskDetailsModalProps> = ({ 
  task, onClose, files, reviews, users, currentUser, onAddFile, onAddReview, onUpdateStatus, allowManagement = true
}) => {
  ...
  const isReviewer = (currentUser.role === UserRole.SUPER_ADMIN || currentUser.role === UserRole.PROJECT_MANAGER) && allowManagement;
      ...
      <div className="modal-dialog modal-lg modal-dialog-centered">
        <div className="modal-content border-0 rounded-4 shadow-lg overflow-hidden">
          <div className="modal-header border-0 pb-0 pt-4 px-4 bg-white">
            <div>
              <span className="badge bg-primary-subtle text-primary mb-2 text-uppercase fw-bold" style={{fontSize: '0.6rem'}}>Task Details</span>
          ...
          <div className="modal-body p-4 bg-white">
            ...
            {activeTab === 'info' && (
              <div className="row g-4">
                ...
                <div className="col-md-4">
                  ...
                  <div className="mb-3">
                    <h6 className="fw-bold text-secondary small text-uppercase mb-2">Status</h6>
                    <select 
                  ...
                  <div className="mb-3">
                    <h6 className="fw-bold text-secondary small text-uppercase mb-2">Due Date</h6>
            ...
            {activeTab === 'files' && (
              <div className="row g-4">
                <div className={`${allowManagement ? 'col-md-7' : 'col-12'}`}>
                  <h6 className="fw-bold mb-3">Uploads</h6>
                  {taskFiles.length === 0 ? (
                    <div className="text-center py-5 border border-dashed rounded-3 text-muted small">No files uploaded yet.</div>
                  ) : (
                    <div className="list-group list-group-flush border rounded-3">
                      {taskFiles.map(file => {
                        ...
                        return (
                          <div key={file.id} className="list-group-item file-item border-bottom-0 p-3">
                            <div className="d-flex justify-content-between align-items-start">
                              <div className="d-flex align-items-center">
                                ...
                                <div>
                                  ...
                                  <div className="smaller text-secondary">Rev {file.revisionNo} • {file.uploadedAt}</div>
                                </div>
                              </div>
                                ...
                                {fileReviews.map(review => (
                                  <div key={review.id} className="smaller p-2 bg-light rounded-2 mb-2">
                                    <div className="d-flex justify-content-between mb-1">
                                      ...
                                      <span className="text-muted">{review.reviewedAt}</span>
                                    </div>
                ...
                {allowManagement && (
                  <div className="col-md-5">
                    <div className="card bg-light border-0 p-3 mb-4">
                      <h6 className="fw-bold mb-3 small text-uppercase text-secondary">Upload New Revision</h6>
                      <form onSubmit={handleFileUpload}>
                        <input name="file" type="file" className="form-control form-control-sm mb-3" required />
                        <button type="submit" className="btn btn-primary btn-sm fw-bold w-100">Upload File</button>
                      </form>
                    </div>

                    {reviewForm.fileId && (
                      <div className="card bg-white border shadow-sm p-3">
                        <h6 className="fw-bold mb-3 small text-uppercase text-primary">Reviewing: {taskFiles.find(f => f.id === reviewForm.fileId)?.filePath}</h6>
                        <form onSubmit={handleReviewSubmit}>
                          <div className="mb-3">
                            <label className="form-label smaller fw-bold">Comments</label>
                            <textarea className="form-control form-control-sm" rows={3} value={reviewForm.comments} onChange={(e) => setReviewForm({...reviewForm, comments: e.target.value})} required></textarea>
                          </div>
                          <div className="mb-3">
                            <label className="form-label smaller fw-bold">Decision</label>
                          ...
                          <div className="d-flex gap-2">
                            <button type="button" className="btn btn-light btn-sm flex-grow-1" onClick={() => setReviewForm({fileId: null, comments: '', status: 'approved'})}>Cancel</button>
                            <button type="submit" className="btn btn-dark btn-sm flex-grow-1">Submit Review</button>
                          </div>
                        </form>
                      </div>
                    )}
          ...
          <div className="modal-footer border-0 p-4 pt-0 bg-white">
            <button className="btn btn-secondary fw-bold px-4" onClick={onClose}>Close</button>
          </div>
        </div>
      </div>
    </div>
