...
import {
  DndContext, closestCorners, KeyboardSensor, PointerSensor, useSensor, useSensors, DragOverlay, DragStartEvent, DragOverEvent, DragEndEvent, defaultDropAnimationSideEffects,
} from '@dnd-kit/core';
import {
  ...
  return (
    ...
    >
      ...
      <div onClick={onClick} className="cursor-pointer">
        ...
        <p className="small text-secondary mb-3 lh-sm" style={{ fontSize: '0.8rem' }}>{task.description.substring(0, 60)}{task.description.length > 60 ? '...' : ''}</p>
      </div>
...
const KanbanColumn: React.FC<{ status: TaskStatus; tasks: Task[]; title: string; onTaskClick: (task: Task) => void }> = ({ status, tasks, title, onTaskClick }) => {
  ...
  return (
    <div className="kanban-col flex-grow-1 mx-2 d-flex flex-column shadow-sm" style={{ backgroundColor: '#eff2f5', minWidth: '300px' }}>
...
const KanbanBoard: React.FC<KanbanBoardProps> = ({ tasks, onTaskUpdate, onTasksReorder, onTaskClick }) => {
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    if (isOverAColumn) {
      if (activeTask.status !== (overId as TaskStatus)) onTaskUpdate(activeId, { status: overId as TaskStatus });
    } else {
      const overTask = tasks.find((t) => t.id === overId);
      if (overTask && activeTask.status !== overTask.status) onTaskUpdate(activeId, { status: overTask.status });
    }
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      if (newIndex !== -1) onTasksReorder(arrayMove(tasks, oldIndex, newIndex));
    }
    setActiveId(null);
  };
  ...
  return (
    <div className="kanban-wrapper" style={{ overflowX: 'auto', paddingBottom: '20px' }}>
      <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
        <div className="d-flex flex-nowrap" style={{ minWidth: '100%', gap: '0.5rem' }}>
          {columns.map(col => (
            <KanbanColumn key={col.status} status={col.status} title={col.title} tasks={tasks.filter(t => t.status === col.status)} onTaskClick={onTaskClick} />
          ))}
        </div>
